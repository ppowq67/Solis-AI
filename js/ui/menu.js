...
// Cache system with ETag support to prevent repeated API calls
const apiCache = {
    userProfile: null,
    userProfileTime: 0,
    ...
    // Track pending requests to prevent duplicate simultaneous fetches
    ...
    // Get pending request promise if one exists (request deduplication)
    getPendingRequest() {
...
async function loadAndSetCurrentUser() {
    try {
        // First check if currentUser already exists in window with full profile fields
        ...
        // Intentionally skip localStorage as a source of truth for currentUser.
        // Stale localStorage data from a previous session causes the "broken dashboard"
        // bug where user name/email appears after logout. The backend session check
        // (the fetch below) is the only authoritative source.
        localStorage.removeItem('currentUser');
        
        // ✅ REQUEST DEDUPLICATION: If a request is already pending, wait for it instead of making another
        ...
        // 🔐 SECURITY: REMOVED localStorage token fallback - only use httpOnly session cookies via credentials: 'include'
        ...
        const fetchPromise = (async () => {
            try {
                ...
                // ✅ HANDLE 401 — global fetch handler already retries/refreshes; don't double-redirect
                ...
                // Always clear pending request when done
...
function updateProfileButton() {
    ...
    let userObj = null;
    try {
        userObj = (typeof window !== 'undefined' && window.currentUser) ? window.currentUser : null;
    } catch (err) {
    ...
    // If user has a profile picture, display it (Google included — proxied via /api/avatar/)
...
function initUserMenu() {
    ...
    if (menuLogout) {
        menuLogout.addEventListener('click', () => {
            ...
            // Wipe all local state immediately so stale data can't leak back
            localStorage.clear();
...
// ============================================================================
// GLOBAL HELPER: Get user profile (prevents redundant fetches from multiple files)
// ============================================================================
...
// Other files (stg.js, qwsqa.js, setup.js) should use this instead of making
// their own /api/user/profile calls. This ensures deduplication and caching.
async function getOrFetchCurrentUser() {
    // If already loaded with full subscription usage fields, return immediately
    if (userProfileIsComplete(window.currentUser)) {
        return window.currentUser;
    }
    
    // Wait for menu.js to load it
    ...
    // Fallback: make the fetch ourselves
