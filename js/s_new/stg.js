// Settings modal — left-nav sections + subscription quotas
document.addEventListener('DOMContentLoaded', () => {
    ...
    const DIR_ACTION_PANELS = new Set(['connectors', 'plugins']);
    let connFilter = 'all';
    let connSearchOpen = false;

    const EFFORT_UI_STORAGE_KEY = 'solis_effort_ui_mode';
    const MAC_CURSOR_KEY = 'solisMacCursor';
    ...
    /** Public helpers for clip generation payload (directory UI is disabled for now) */
    ...
    function readMacCursorMode() {
        try {
            const v = localStorage.getItem(MAC_CURSOR_KEY);
            if (v === 'off' || v === 'preview' || v === 'always') return v;
        } catch (_) {}
        return 'preview';
    }

    function persistMacCursorMode(mode) {
        try { localStorage.setItem(MAC_CURSOR_KEY, mode); } catch (_) {}
    }

    function applyMacCursorMode(mode) {
        const next = (mode === 'off' || mode === 'always') ? mode : 'preview';
        persistMacCursorMode(next);
        document.documentElement.setAttribute('data-solis-cursor', next);
        syncMacCursorUi();
    }

    function syncMacCursorUi() {
        const mode = readMacCursorMode();
        const on = mode !== 'off';
        const scope = mode === 'always' ? 'always' : 'preview';
        const toggle = document.getElementById('stgMacCursorToggle');
        const label = document.getElementById('stgMacCursorLabel');
        const scopeRow = document.getElementById('stgMacCursorScopeRow');
        if (toggle) {
            toggle.classList.toggle('is-on', on);
            toggle.setAttribute('aria-checked', on ? 'true' : 'false');
        }
        if (label) {
            label.textContent = !on ? 'Off' : (scope === 'always' ? 'Everywhere' : 'Preview');
        }
        if (scopeRow) scopeRow.classList.toggle('is-disabled', !on);
        document.querySelectorAll('[data-solis-cursor-scope]').forEach((btn) => {
            const active = on && btn.getAttribute('data-solis-cursor-scope') === scope;
    ...
    function syncDirMainActions(panelId) {
        const actions = document.getElementById('stgMainActions');
        const browse = document.getElementById('stgDirBrowseBtn');
        if (!actions) return;
        const show = DIR_ACTION_PANELS.has(panelId);
        actions.hidden = !show;
        if (browse) {
            browse.textContent = panelId === 'plugins' ? 'Browse' : 'Browse';
            browse.disabled = panelId === 'plugins';
            browse.title = panelId === 'plugins' ? 'Coming soon' : '';
        }
        const searchBar = document.getElementById('stgConnSearchBar');
        if (searchBar) {
            const open = show && panelId === 'connectors' && connSearchOpen;
            searchBar.hidden = !open;
        }
    }

    function filterConnectorRows() {
        const q = (document.getElementById('stgConnectorsSearch')?.value || '').trim().toLowerCase();
        const rows = document.querySelectorAll('#stgConnTable .stgConnRow');
        const popular = document.querySelectorAll('.stgConnPopularCard');
        let visible = 0;
        rows.forEach((row) => {
            const status = row.getAttribute('data-conn-status') || 'off';
            const name = (row.getAttribute('data-conn-name') || row.getAttribute('data-conn-id') || '').toLowerCase();
            const statusOk =
                connFilter === 'all' ||
                (connFilter === 'connected' && status === 'on') ||
                (connFilter === 'off' && status !== 'on');
        ...
        popular.forEach((card) => {
            const status = card.getAttribute('data-conn-status') || 'off';
            const id = (card.getAttribute('data-conn-id') || '').toLowerCase();
            const statusOk =
                connFilter === 'all' ||
                (connFilter === 'connected' && status === 'on') ||
                (connFilter === 'off' && status !== 'on');
        ...
        const empty = document.getElementById('stgConnEmpty');
        if (empty) empty.hidden = visible > 0;
        const popularWrap = document.querySelector('.stgConnPopular');
        if (popularWrap) {
            const anyPopular = [...popular].some((c) => !c.hidden);
            popularWrap.hidden = !anyPopular;
        }
    }

    function setConnFilter(next) {
        connFilter = next === 'connected' || next === 'off' ? next : 'all';
        document.querySelectorAll('.stgConnFilter').forEach((btn) => {
            const on = btn.getAttribute('data-conn-filter') === connFilter;
    ...
    function filterSettingsNav(query) {
        ...
        document.querySelectorAll('.stgNavGroup').forEach((group) => {
            let any = false;
            group.querySelectorAll('.stgNavBtn[data-stg-panel]').forEach((btn) => {
                const hay = `${btn.getAttribute('data-stg-search') || ''} ${btn.textContent || ''}`.toLowerCase();
                const match = !q || hay.includes(q);
                btn.hidden = !match;
                if (match) any = true;
            });
            group.setAttribute('data-stg-empty', any ? 'false' : 'true');
        });
    }

    function switchSettingsPanel(panelId) {
        let id = panelId;
        // Connectors / Plugins coming soon — never open
        ...
        document.querySelectorAll('.stgPanel').forEach((panel) => {
            ...
            if (on && isMobileSettings()) {
                // Retrigger enter animation when switching sections on mobile
                void panel.offsetWidth;
                panel.classList.add('stg-panel-enter');
            }
        });
        if (stgMainTitle) stgMainTitle.textContent = PANEL_TITLES[id] || 'Settings';
        syncDirMainActions(id);
        if (id === 'connectors') filterConnectorRows();
    ...
    function setSettingsMobileView(view) {
        ...
        // Ensure first paint has a starting state so the slide can run
        if (!prev) {
    ...
    const macCursorToggle = document.getElementById('stgMacCursorToggle');
    if (macCursorToggle) {
        applyMacCursorMode(readMacCursorMode());
        macCursorToggle.addEventListener('click', () => {
            const cur = readMacCursorMode();
            if (cur === 'off') {
                let scope = 'preview';
                try {
                    const last = localStorage.getItem('solisMacCursorScope');
                    if (last === 'always' || last === 'preview') scope = last;
                } catch (_) {}
                applyMacCursorMode(scope);
            } else {
                try { localStorage.setItem('solisMacCursorScope', cur === 'always' ? 'always' : 'preview'); } catch (_) {}
    ...
    document.querySelectorAll('[data-solis-cursor-scope]').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            const scope = btn.getAttribute('data-solis-cursor-scope') === 'always' ? 'always' : 'preview';
            try { localStorage.setItem('solisMacCursorScope', scope); } catch (_) {}
            applyMacCursorMode(scope);
        });
    });
    syncMacCursorUi();
    ...
    function openSettingsModal(panel) {
        ...
        // Set view BEFORE open so the first painted frame can slide correctly
        if (isMobileSettings()) {
        ...
        // Next frame: commit open class so sheet / backdrop transitions run
    ...
    async function fetchSecureSettingsData() {
        ...
        // Debloated: identity + quotas + billing (for cancel)
        ...
        // Mirror billing fields onto profile for cancel UI
    ...
    async function cancelSubscriptionViaPaddle() {
        ...
        // Immediate action: renewals stop as soon as you confirm. Warn clearly.
    ...
    function formatQuotaResetHint(iso, fallback) {
        ...
        const ts = Date.parse(hasTz ? raw : `${raw.replace(' ', 'T')}Z`);
        if (Number.isNaN(ts)) {
            return String(fallback || '').replace(/\s*Resets \{when\}\.?/i, '').trim()
                || 'Usage details unavailable.';
        }
    ...
    async function updateSettingsModal() {
        ...
        setText('stgName', cachedUser?.name || cachedUser?.username || 'Guest User');
        if (window.SolisBadges?.renderCurrentUser) {
            window.SolisBadges.renderCurrentUser('stgBadges', 20);
        }
        setText('stgUserEmail', cachedUser?.email || 'unknown@email.com');
        setText('stgEmailAddress', cachedUser?.email || 'unknown@email.com');
        const stgBio = document.getElementById('stgBio');
        const hero = document.getElementById('stgProfileHero');
        if (stgBio && !hero?.classList.contains('is-editing')) {
            stgBio.textContent = cachedUser?.bio || '';
        }

        const stgAvatar = document.getElementById('stgAvatar');
        if (stgAvatar) {
            const pictureUrl = typeof resolveAvatarUrl === 'function'
                ? resolveAvatarUrl(cachedUser || {})
                : (cachedUser?.picture || cachedUser?.avatar || null);
        ...
        // Skeletons immediately — panel stays visible, no artificial wait
        setSubscriptionLoading(true);

        // Optimistic plan label from cache (hidden under skeleton until ready)
        const cachedPlan = String(cachedUser?.plan || window.currentUser?.plan || 'free').toLowerCase();
        ...
        try {
            ...
            // Keep identity fields in sync with server (name / bio / avatar)
            ...
            if (profile.picture || profile.avatar) {
                ...
                if (stgAvatarEl && okUrl) {
                    ...
                    // Bust immutable/stale caches after server avatar changes
            ...
            // Prefer /api/clips/status (same source as storage badge + Max quota)
            ...
            const libraryUnlimited = statusVideos.unlimited === true
                || ['basic', 'prime', 'elite'].includes(String(actualPlan || '').toLowerCase());
            const videosInLibrary = Math.max(
                0,
                Number(statusVideos.used ?? 0) || 0
            );
            const videoLimit = libraryUnlimited
                ? null
                : Math.max(1, Number(statusVideos.limit ?? 5) || 5);
            if (libraryUnlimited) {
                setText('stgVideosUsed', videosInLibrary + ' clips (unlimited)');
            ...
            // Clip count is the limit — hide legacy GB storage meter
            const storageQuota = document.getElementById('stgStorage')?.closest('.stgQuota');
            if (storageQuota) storageQuota.hidden = true;

            // Daily generations — primary hard limit
                    ...
                    if (statusDaily.resets_at) {
                        dailyHint.textContent = formatQuotaResetHint(
                            ...
                            dailyUsed >= dailyLimit
                                ? 'Daily limit reached. Resets {when}.'
                                : 'Resets {when}.'
                        );
                    }
                }
                ...
                if (monthlyHint) {
                    monthlyHint.textContent = formatQuotaResetHint(
                        ...
                        monthlyUsed >= monthlyLimit
                            ? 'Monthly limit reached. Resets {when}.'
                            : 'Resets {when}.'
                    );
                }
            } else {
        ...
        updateYouTubeConnectorUI(!!(window.currentUser?.youtube_connected || cachedUser?.youtube_connected));
    }

    function updateYouTubeConnectorUI(connected) {
        const statusEl = document.getElementById('stgYouTubeStatus');
        const btns = [
            document.getElementById('stgYouTubeConnectBtn'),
            document.getElementById('stgYouTubeConnectBtnPopular'),
        ].filter(Boolean);
        const row = document.getElementById('stgYouTubeConnector');
        const popular = document.querySelector('.stgConnPopularCard[data-conn-id="youtube"]');
        ...
        if (row) {
            ...
            row.setAttribute('data-conn-status', status);
        }
        if (popular) popular.setAttribute('data-conn-status', status);
        filterConnectorRows();
    }
    ...
    ['stgYouTubeConnectBtn', 'stgYouTubeConnectBtnPopular'].forEach((id) => {
        document.getElementById(id)?.addEventListener('click', handleYouTubeConnectClick);
    });

    document.querySelectorAll('.stgConnFilter').forEach((btn) => {
        btn.addEventListener('click', () => setConnFilter(btn.getAttribute('data-conn-filter')));
    });
    ...
    document.getElementById('stgDirSearchToggle')?.addEventListener('click', () => {
        const active = document.querySelector('.stgNavBtn.is-active[data-stg-panel]')?.getAttribute('data-stg-panel');
        if (active !== 'connectors') return;
        connSearchOpen = !connSearchOpen;
        const searchBar = document.getElementById('stgConnSearchBar');
        if (searchBar) {
            searchBar.hidden = !connSearchOpen;
            if (connSearchOpen) document.getElementById('stgConnectorsSearch')?.focus();
        }
    });
    ...
    document.getElementById('stgDirAddBtn')?.addEventListener('click', () => {
        /* placeholder — custom connector add coming later */
    });
    ...
    async function saveProfileChanges(newName, newBio) {
        ...
        try {
            ...
            document.querySelectorAll('.user-name, #menuUserName').forEach((el) => {
                if (el) el.textContent = savedName;
            });
            const ddName = document.querySelector('#dropdownUserName .username-text');
            if (ddName) ddName.textContent = savedName;
            else {
                const dd = document.getElementById('dropdownUserName');
                if (dd && !dd.querySelector('.badge-container')) dd.textContent = savedName;
            }
            try {
    ...
    // ── Secure profile picture upload + centered crop adjuster ──
    ...
    function openCropModal(file) {
        if (!cropModal || !cropImg || !cropViewport) {
            // Fallback: upload without adjuster
    ...
    /** Export the circular viewport as a sanitized square WEBP/JPEG file. */
    async function exportCroppedAvatarFile() {
        ...
        // Viewport center shows image point at (-offset/scale + natural/2) roughly
    ...
    function applyAvatarEverywhere(url) {
        ...
        try {
            if (typeof resolveAvatarUrl === 'function' && window.currentUser) {
                resolved = resolveAvatarUrl(window.currentUser.id || window.currentUser, url) || url;
            } else if (typeof resolveAvatarUrl === 'function') {
                resolved = resolveAvatarUrl(null, url) || url;
            }
        } catch (_) { /* ignore */ }
        // Relative /api/avatar must hit API host (not Vercel)
        if (resolved.startsWith('/') && typeof window.apiUrl === 'function') {
            try {
                const origin = String(window.apiUrl('/')).replace(/\/api\/?$/, '');
                if (origin) resolved = origin + resolved;
            } catch (_) { /* ignore */ }
        } else if (resolved.startsWith('/') && window.API_BASE_URL) {
            const origin = String(window.API_BASE_URL).replace(/\/api\/?$/, '');
            if (origin) resolved = origin + resolved;
        }
        const bust = resolved + (resolved.includes('?') ? '&' : '?') + 't=' + Date.now();
        const setImg = (root) => {
            if (!root) return;
    ...
    async function uploadProfilePicture(file) {
        ...
        try {
            ...
            // Keep relative proxy path in user object; resolveAvatarUrl absolutizes for display
            if (window.currentUser && typeof window.currentUser === 'object') {
    ...
    async function copySupportEmail(btn) {
        const email = (btn?.dataset?.email || '').trim();
        if (!email) return;
        try {
            await navigator.clipboard.writeText(email);
            const value = btn.querySelector('.stgRowValue');
            if (value) {
                const prev = value.textContent;
                value.textContent = 'Copied';
                setTimeout(() => { value.textContent = prev; }, 1400);
            }
        } catch (_) {
