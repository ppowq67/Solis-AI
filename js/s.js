...
const PreviewTimeline = (() => {
  ...
  const t = (() => {
    try {
      ...
      const e = Number(navigator.deviceMemory) || 0;
      const t = Number(navigator.hardwareConcurrency) || 0;
      if (e > 0 && e <= 4) return true;
      if (t > 0 && t <= 4) return true;
      if (/Android|iPhone|iPad|Mobile/i.test(navigator.userAgent || "") && t > 0 && t <= 6) return true;
    } catch (e) {}
    return false;
  })();
  const i = t ? 8 : 14;
  ...
  let ye;
  function refreshEls() {
    ie = document.getElementById("previewTimelineShell");
    ne = document.getElementById("previewTimelineWrap");
    re = document.getElementById("previewTimelineCurrent");
    se = document.getElementById("previewTimelineDuration");
    oe = ne;
    ae = document.getElementById("previewTimelineFilmstrip");
    le = document.getElementById("previewTimelineSegments");
    ce = document.getElementById("previewTimelineDimLeft");
    de = document.getElementById("previewTimelineDimRight");
    ue = document.getElementById("previewTimelineSelection");
    pe = document.getElementById("previewTimelineRangePick");
    me = document.getElementById("previewTimelineHandleL");
    fe = document.getElementById("previewTimelineHandleR");
    ye = document.getElementById("previewTimelinePlayhead");
  }
  function setHandlesUnlocked(e) {
    $ = !!e;
    if (ie) ie.classList.toggle("handles-on", $);
  }
  function setRankingEditMode(e) {
    refreshEls();
    if (ie) ie.classList.toggle("is-ranking-edit", !!e);
    if (e) setHandlesUnlocked(true);
    if (!e) ee = [ 5, 4, 3, 2, 1 ];
  }
  function isRankingEdit() {
    return !!ie?.classList.contains("is-ranking-edit");
  }
  function getClipOrder() {
    const e = Math.max(1, getSegmentBounds().length - 1);
    while (ee.length < e) {
      const e = Math.max(1, 5 - ee.length);
      if (!ee.includes(e)) ee.push(e); else ee.push(ee.length + 1);
    }
    return ee.slice(0, e);
  }
  ...
  function applySegmentReorder(t, i) {
    ...
    try {
      window.clipsStudio?.onRankingClipReorder?.(ee.slice());
    } catch (e) {}
    return true;
  }
  function paintSegmentReorderGhost(e) {
    if (!z || !isRankingEdit()) return;
    const t = segmentIndexAtClientX(e);
    z.hoverIndex = t;
    const i = Array.from(le?.children || []);
    i.forEach((e, i) => {
      e.classList.toggle("is-drop-target", i === t && i !== z.index);
      e.classList.toggle("is-dragging", i === z.index);
    });
    const n = e - z.startX;
    const r = i[z.index];
    if (r) {
  ...
  function cacheKey(e, t) {
    return `${e}|${Math.round(t * 10) / 10}|${i}`;
  }
  function rememberCache(e, t) {
    if (l.has(e)) l.delete(e);
    l.set(e, t);
    while (l.size > a) {
  ...
  function cacheTrackMetrics() {
    ...
    P = e.left;
    E = Math.max(1, e.width);
  }
  function timeFromClientX(e) {
    if (!f) return 0;
    const t = (e - P) / E;
    return Math.max(0, Math.min(f, t * f));
  }
  function paintPlayheadOnly() {
    if (re) re.textContent = fmt(y);
    if (!f || E <= 0 || !ye) return;
    const e = Math.max(p, Math.min(m, y));
    const t = e / f * E;
    ye.style.transform = `translate3d(${t}px,0,0) translateX(-50%)`;
  }
  function paintChrome({rebuildSegments: e = false} = {}) {
    ...
    const t = u === "start" || u === "end" || u === "bound" || u === "segment";
    if (!t) {
      if (re) re.textContent = fmt(y);
      if (se) se.textContent = fmt(f || 0);
    }
    if (!f || E <= 0) return;
    const i = p / f * E;
    const n = m / f * E;
    ...
    if (ue) {
      ue.style.width = `${r}px`;
      ue.style.transform = `translate3d(${i}px,0,0)`;
    }
    if (ce) ce.style.width = `${i}px`;
    if (de) de.style.width = `${Math.max(0, E - n)}px`;
    if (ye && !t) {
      const e = Math.max(p, Math.min(m, y));
      const t = e / f * E;
      ye.style.transform = `translate3d(${t}px,0,0) translateX(-50%)`;
    ...
    if (!t && oe) {
      oe.setAttribute("aria-valuenow", String(Math.round(y / f * 100)));
      oe.setAttribute("aria-valuetext", fmt(y));
    }
  ...
  function paintCutPreview() {
    if (!ne || !f || E <= 0) return;
    let e = ne.querySelector(".preview-timeline-cut-preview");
    if (!A.length) {
      he = "";
      if (e) e.remove();
      if (ne) {
        ne.classList.remove("has-cut-preview", "has-cut-preview-add");
      }
      return;
    }
    const t = `${E}|${A.map(e => `${e.kind || "remove"}:${e.start}:${e.end}`).join(",")}`;
    if (e && t === he) return;
    he = t;
    const i = A.some(e => (e.kind || "remove") !== "add");
    const n = A.some(e => e.kind === "add");
    ne.classList.toggle("has-cut-preview", i);
    ne.classList.toggle("has-cut-preview-add", n && !i);
    if (!e) {
    ...
    for (const e of A) {
      const t = e.start / f * E;
      const i = Math.max(2, (e.end - e.start) / f * E);
  ...
  function paintSilenceCuts() {
    if (!ne || !f || E <= 0) return;
    let e = ne.querySelector(".preview-timeline-silence-cuts");
    if (!B.length) {
      ge = "";
      if (e) e.remove();
      return;
    }
    const t = `${E}|${B.map(e => `${e.start}:${e.end}`).join(",")}`;
    if (e && t === ge) return;
    ge = t;
    if (!e) {
      e = document.createElement("div");
    ...
    for (const e of B) {
      const t = e.start / f * E;
      const n = Math.max(2, (e.end - e.start) / f * E);
  ...
  function cloneFilmInto(e) {
    if (!ae) return;
    const t = ae.querySelectorAll(".preview-timeline-frame");
    if (!t.length) {
      for (let t = 0; t < i; t++) {
  ...
  function getSegmentBounds() {
    const e = x.filter(e => e > p + .04 && e < m - .04);
    return [ p, ...e, m ];
  }
  ...
  function syncSegFocusClass() {
    if (!ie) return;
    const e = M != null && getSegmentBounds().length > 2;
    ie.classList.toggle("has-seg-focus", e);
  }
  function getActiveEditRange() {
    if (W && Number.isFinite(W.start) && Number.isFinite(W.end) && W.end - W.start > .35) {
      return {
        start: W.start,
        end: W.end,
        segIndex: "range",
  ...
  function paintManualRange() {
    ...
    if (!W || !e) {
      if (!pe.hidden && pe.classList.contains("is-live")) {
        ...
        J = setTimeout(() => {
          ...
          ne?.classList.remove("is-range-picking");
        }, 460);
        return;
      }
      pe.hidden = true;
    ...
    const t = Math.abs(W.end - W.start);
    if (t < .12 && !X) {
      pe.hidden = true;
      pe.setAttribute("hidden", "");
      pe.setAttribute("aria-hidden", "true");
    ...
    const i = ne?.clientWidth || ne?.getBoundingClientRect?.()?.width || 0;
    if (!i) {
      requestAnimationFrame(() => paintManualRange());
      return;
    }
    const n = Math.max(0, Math.min(W.start, e)) / e;
    const r = Math.max(0, Math.min(W.end, e)) / e;
  ...
  function clearManualRange() {
    ...
    try {
      window.SolisImproveClip?.dismissSelection?.();
    } catch (e) {
      try {
        window.SolisImproveClip?.close?.();
      } catch (e) {}
    }
    try {
  ...
  function timeFromClientX(e) {
    const t = ne?.getBoundingClientRect?.();
  ...
  function highlightPartAt(e, t = {}) {
    const i = effectiveDuration() || f;
    if (!(i > .25)) return null;
    const n = getSegmentBounds();
    let r;
    let s;
    let o = t.segIndex;
    if (o != null && o >= 0 && o < n.length - 1) {
      r = n[o];
      s = n[o + 1];
    } else if (n.length > 2 && Number.isFinite(e)) {
      const t = Number(e);
      o = null;
      for (let e = 0; e < n.length - 1; e++) {
        const i = n[e];
        const a = n[e + 1];
        if (t >= i - .02 && (e === n.length - 2 ? t <= a + .02 : t < a - .01)) {
          r = i;
          s = a;
          o = e;
          break;
    ...
    if (!(Number.isFinite(r) && Number.isFinite(s) && s - r >= .35)) {
      const t = Math.max(p, Math.min(m || i, Number(e) || y || 0));
      const n = 1.75;
      r = Math.max(p, t - n);
      s = Math.min(m > p ? m : i, t + n);
      if (s - r < .35) {
        r = Math.max(p, Math.min(r, (m || i) - .35));
        s = Math.min(m > p ? m : i, r + Math.max(.35, Math.min(3.5, i)));
      }
      o = null;
    }
    M = o != null ? o : null;
    syncSegFocusClass();
    le?.querySelectorAll(".preview-timeline-segment.is-selected").forEach(e => e.classList.remove("is-selected"));
    if (M != null && le?.children?.[M]) {
      le.children[M].classList.add("is-selected");
    }
    const a = setManualRange(r, s);
    y = Math.max(r, Math.min(s - .05, r + Math.min(.35, (s - r) * .2)));
    scheduleSeek(y, true);
    paintChrome();
    return a;
  }
  function customizeSegment(e) {
    ...
    y = Math.max(i, Math.min(n - .05, r));
    scheduleSeek(y, true);
    paintChrome();
    le?.querySelectorAll(".preview-timeline-segment.is-selected").forEach(e => e.classList.remove("is-selected"));
    le?.children?.[e]?.classList.add("is-selected");
    if (!$) setHandlesUnlocked(true);
    syncSegFocusClass();
    try {
  ...
  function applySegmentTimes(t, i, n) {
    ...
    const o = t >= r.length - 2 ? f : r[t + 2] - e;
    ...
    if (t === r.length - 2) m = c; else {
      const e = r[t + 1];
      const i = x.findIndex(t => Math.abs(t - e) < .05);
      if (i >= 0) x[i] = c; else x.push(c);
    }
    x = x.filter(e => e > p + .04 && e < m - .04).sort((e, t) => e - t);
  }
  function paintSegmentMove(e) {
    ...
    const {index: t, startX: i, startA: n, startB: r} = z;
    const s = (e - i) / E * f;
    applySegmentTimes(t, n + s, r + s);
    y = Math.max(p, Math.min(m, n + s));
    const o = p / f * E;
    const a = m / f * E;
    if (ue) {
      ue.style.width = `${Math.max(2, a - o)}px`;
      ue.style.transform = `translate3d(${o}px,0,0)`;
    }
    if (ce) ce.style.width = `${o}px`;
    if (de) de.style.width = `${Math.max(0, E - a)}px`;
    paintSegmentsFast();
  }
  function beginSegmentDrag() {
    if (!z || u === "segment") return;
    clearTimeout(j);
    j = 0;
    ...
    if (isRankingEdit()) oe?.classList.add("is-reordering");
    z.target?.classList.add("is-dragging");
    le?.children?.[z.index]?.classList.add("is-dragging");
    if (z.target?.setPointerCapture && z.pointerId != null) {
      try {
  ...
  function onSegmentPointerDown(e, t) {
    ...
    t.preventDefault();
    t.stopPropagation();
    if (!$ && !ie?.classList.contains("is-ranking-edit")) {
    ...
    j = setTimeout(() => {
      j = 0;
      G = true;
      z?.target?.classList.add("is-hold-ready");
    }, V);
  }
  function onSegmentPointerMove(e) {
    if (Y) return;
    if (Z && Math.abs(e.clientX - (Z.startX || 0)) >= q) {
      return;
    }
    if (!z) return;
    const t = Math.abs(e.clientX - z.startX);
    if (u === "segment") {
      e.preventDefault();
      if (t >= q) H = true;
      if (!H) return;
  ...
  function onSegmentPointerUp(e) {
    ...
    le?.children?.[t.index]?.classList.remove("is-dragging");
    le?.querySelectorAll(".is-drop-target").forEach(e => e.classList.remove("is-drop-target"));
    oe?.classList.remove("is-reordering");
    if (u === "segment" && !i) {
      u = null;
      z = null;
      oe?.classList.remove("is-dragging", "is-trimming");
      if (h) c?.play().catch(() => {});
      h = false;
  ...
  function paintSegmentsFast() {
    if (!le || !f || E <= 0) return;
    const e = Array.from(le.children);
    const t = getSegmentBounds();
  ...
  function applyBoundTime(e) {
    ...
    if (N != null) {
      const t = x.findIndex(e => Math.abs(e - N) < .05);
  ...
  function paintBoundFast(e) {
    ...
    const i = Math.max(D, Math.min(F, t));
    applyBoundTime(i);
    y = i;
    const n = p / f * E;
    const r = m / f * E;
    if (ue) {
      ue.style.width = `${Math.max(2, r - n)}px`;
      ue.style.transform = `translate3d(${n}px,0,0)`;
    }
    if (ce) ce.style.width = `${n}px`;
    if (de) de.style.width = `${Math.max(0, E - r)}px`;
    paintSegmentsFast();
  }
  function startBoundDrag(t, i) {
    if (!$ && !ie?.classList.contains("is-ranking-edit")) return;
    if (!$) setHandlesUnlocked(true);
  ...
  function paintTrimFast(t) {
    ...
    if (u === "start") {
      p = Math.max(0, Math.min(i, m - e));
    } else if (u === "end") {
      m = Math.min(f, Math.max(i, p + e));
    } else {
      return;
    }
    const n = p / f * E;
  ...
  function scheduleSeek(e, t = false) {
    if (!c || !Number.isFinite(e)) return;
    k = Math.max(0, Math.min(f || e, e));
    const i = performance.now();
    const n = u === "scrub" ? o : s;
    const r = t ? 0 : Math.max(0, n - (i - L));
    if (T) {
      clearTimeout(T);
      T = 0;
    }
  ...
  function flushSeek() {
    if (!c || k == null || C) return;
    const e = k;
    k = null;
    if (Math.abs((c.currentTime || 0) - e) < .012) {
      if (!u) {
        y = c.currentTime || e;
        paintChrome();
      }
    ...
    const onSeeked = () => {
      C = false;
      c.removeEventListener("seeked", onSeeked);
      if (!u) {
        y = c.currentTime || y;
        paintChrome();
      }
    ...
    c.addEventListener("seeked", onSeeked);
    try {
      c.currentTime = e;
    } catch (e) {
      C = false;
  ...
  async function resolveCaptureSrc(e) {
    ...
    try {
      ...
      try {
        window.LibraryPreviewMediaCache?.rememberSrc?.(e, n, b);
      } catch (e) {}
      return b;
    } catch (t) {
      return e;
  ...
  async function buildFilmstripFromVideo() {
    if (!ae || !c) return;
    const e = c.currentSrc || c.src;
    if (!e || !f || f <= 0) {
      buildPlaceholderFilmstrip();
      return;
    }
    const t = cacheKey(e, f);
    const s = l.get(t);
    if (s?.length) {
    ...
    a.src = await resolveCaptureSrc(e);
    if (o !== g) {
      destroyCaptureVideo();
      return;
    }
    w = a;
    try {
      await Promise.race([ waitEvent(a, "loadeddata", 8e3), waitEvent(a, "loadedmetadata", 8e3) ]);
      for (let e = 0; e < 20 && !(a.videoWidth > 2); e++) {
        await new Promise(e => setTimeout(e, 50));
      }
      if (o !== g) return;
      const e = Math.max(.01, f - .04);
      const s = [];
      for (let t = 0; t < i; t++) {
        if (o !== g) return;
        const l = t / Math.max(1, i - 1) * e;
        await seekCapture(a, l);
        if (o !== g) return;
        ...
        d.fillStyle = "#ffffff";
        d.fillRect(0, 0, n, r);
        const u = a.videoWidth || 0;
        const p = a.videoHeight || 0;
        if (u > 2 && p > 2) {
          const e = Math.max(n / u, r / p);
          const t = u * e;
          const i = p * e;
          try {
            d.drawImage(a, (n - t) / 2, (r - i) / 2, t, i);
          } catch (e) {}
        }
        s.push(c);
      }
      ...
      if (w === a) destroyCaptureVideo();
      if (o === g && !ae?.querySelector("canvas")) {
        paintSegments();
      }
    }
  }
  function resolveSkipTime(e) {
    if (!B.length || !Number.isFinite(e)) return null;
  ...
  function endDrag() {
    ...
    if (!u) return;
    const e = u;
    oe?.classList.remove("is-scrubbing", "is-trimming", "is-dragging");
    le?.querySelectorAll(".preview-timeline-handle.is-dragging").forEach(e => e.classList.remove("is-dragging"));
    me?.classList.remove("is-dragging");
    fe?.classList.remove("is-dragging");
    if (_ != null) {
      applyPointer(_, {
        seek: false
      });
  ...
  function startDrag(e, t) {
    if ((e === "start" || e === "end" || e === "bound") && !$ && !ie?.classList.contains("is-ranking-edit")) return;
    if ((e === "start" || e === "end" || e === "bound") && !$) {
      setHandlesUnlocked(true);
    }
    if (t.pointerType === "mouse" && t.button !== 0) return;
    ...
    h = c ? !c.paused : false;
    if (c && !c.paused) c.pause();
    oe?.classList.add("is-dragging");
    if (e === "scrub") oe?.classList.add("is-scrubbing"); else oe?.classList.add("is-trimming");
    if (e === "start") me?.classList.add("is-dragging");
    if (e === "end") fe?.classList.add("is-dragging");
  ...
  function bindEvents() {
    ...
    const onMeta = () => {
      const e = Number.isFinite(c.duration) ? c.duration : 0;
      const t = Math.abs(e - f) > .05;
      f = e;
      if (t || m <= p) {
        p = 0;
        m = f || 1;
    ...
    c.addEventListener("durationchange", onMeta);
    ...
    te.push(() => {
      ...
      c.removeEventListener("durationchange", onMeta);
      window.removeEventListener("resize", onResize);
    });
    const beginRangeAt = (e, t) => {
      refreshEls();
      if (!(effectiveDuration() > .25)) {
        seedDuration(f > .25 ? f : m > .25 ? m : 15, {
          rebuildSegments: true
        });
      }
      cacheTrackMetrics();
      ...
      try {
        ne?.setPointerCapture?.(t);
      } catch (e) {}
    };
    const clearLeftRangePending = () => {
      if (K) {
        clearTimeout(K);
        K = 0;
      }
    ...
    oe?.addEventListener("pointerdown", onTrackDown, true);
    const onTrackDblClick = e => {
      ...
      e.stopPropagation();
      cacheTrackMetrics();
      highlightPartAt(timeFromClientX(e.clientX));
    };
    oe?.addEventListener("dblclick", onTrackDblClick);
    ne?.addEventListener("dblclick", onTrackDblClick);
    ...
    oe?.addEventListener("contextmenu", e => {
      if (Y || X) {
        e.preventDefault();
        e.stopPropagation();
        try {
          window.PreviewCtxMenu?.suppressNext?.(600);
        } catch (e) {}
        return;
      }
      if (Z && Z.button === 2) {
    ...
    me?.addEventListener("pointerdown", onStartDown);
    fe?.addEventListener("pointerdown", onEndDown);
    ...
    const onPreviewClickToggle = e => {
      if (!(e.target instanceof Element)) return;
      if (e.target.closest('button, a, input, textarea, select, [contenteditable="true"], ' + ".preview-timeline-row, .improve-edit-bar, #improveEditBar, " + ".solis-subtitle, .caption-word, .overlay-text, " + "[data-template-element-id], .ranking-text-pill, .rk-suggest-actions")) return;
      if (window.getSelection?.()?.toString?.()) return;
      const i = t?.querySelector?.("video");
      if (!i) return;
      e.preventDefault();
      if (i.paused) i.play().catch(() => {}); else i.pause();
    };
    if (t && t.dataset.ytClickBound !== "1") {
      t.dataset.ytClickBound = "1";
      t.addEventListener("click", onPreviewClickToggle);
      te.push(() => {
        t.removeEventListener("click", onPreviewClickToggle);
        delete t.dataset.ytClickBound;
      });
    }
    const onRangeMove = e => {
      if (!Y && Z && e.pointerId === Z.pointerId) {
        const t = Math.abs(e.clientX - Z.startX);
        if (Z.held && t >= q) {
          const e = Z;
          clearLeftRangePending();
          beginRangeAt(e.startX, e.pointerId);
        }
      }
      if (!Y) return;
      e.preventDefault();
      const t = timeFromClientX(e.clientX);
      if (Math.abs(t - Q) > .08) X = true;
      const i = Math.min(Q, t);
      const n = Math.max(Q, t);
      W = {
        start: i,
        end: n
      };
    ...
    const onRangeUp = e => {
      if (Z && e.pointerId === Z.pointerId) {
        const t = Z;
        clearLeftRangePending();
        if (!Y && t.button === 2 && !t.openedCtx) {
          try {
            const i = ne || document.getElementById("previewTimelineWrap");
            window.PreviewCtxMenu?.open?.({
              preventDefault() {},
              stopPropagation() {},
              clientX: t.startX,
        ...
        if (!Y && t.button === 0) {
          const i = Math.abs((e.clientX || 0) - t.startX) >= q;
          if (!i) {
            try {
              const e = timeFromClientX(t.startX);
              if (Number.isFinite(e)) {
                y = Math.max(p, Math.min(m || f || e, e));
                if (c) {
                  try {
                    c.currentTime = y;
                  } catch (e) {}
    ...
    const onOutsideRangeClear = e => {
      ...
      if (!W && !window.SolisImproveClip?.isOpen?.()) return;
      clearManualRange();
    };
    window.addEventListener("pointerdown", onOutsideRangeClear, true);
    window.addEventListener("mousedown", onOutsideRangeClear, true);
    te.push(() => {
      clearLeftRangePending();
      oe?.removeEventListener("pointerdown", onTrackDown, true);
      oe?.removeEventListener("dblclick", onTrackDblClick);
      ne?.removeEventListener("dblclick", onTrackDblClick);
      ...
      me?.removeEventListener("pointerdown", onStartDown);
      fe?.removeEventListener("pointerdown", onEndDown);
    ...
    re?.addEventListener("pointerdown", onTimeChipDown);
    se?.addEventListener("pointerdown", onTimeChipDown);
    te.push(() => {
      re?.removeEventListener("pointerdown", onTimeChipDown);
      se?.removeEventListener("pointerdown", onTimeChipDown);
    });
    const onKey = e => {
      if (!c || !f) return;
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        ...
        y = Math.max(p, Math.min(m, (c.currentTime || 0) + t));
        paintChrome();
        scheduleSeek(y, true);
      }
    };
    oe?.addEventListener("keydown", onKey);
    te.push(() => oe?.removeEventListener("keydown", onKey));
    const i = document.getElementById("previewTimelineSplitBtn");
    const onSplitClick = e => {
      e.preventDefault();
      e.stopPropagation();
      const t = Number.isFinite(c?.currentTime) ? c.currentTime : y;
      const n = splitAt(t);
      if (!n && x.length >= 4) {
    ...
    i?.addEventListener("click", onSplitClick);
    te.push(() => i?.removeEventListener("click", onSplitClick));
  }
  function show() {
    const e = document.getElementById("previewTimelineRow");
    if (e) e.hidden = false;
    if (ie) ie.hidden = false;
    const t = document.getElementById("previewAudioToggle");
    if (t) t.hidden = false;
    try {
  ...
  function detach() {
    ...
    $ = false;
    if (ie) {
      ie.classList.remove("handles-on");
      ie.classList.remove("is-ranking-edit");
      ie.classList.remove("has-silence-cuts");
      ie.classList.remove("has-seg-focus");
    }
    if (ne) {
      ne.classList.remove("has-cut-preview", "has-cut-preview-add");
      ne.classList.remove("is-range-picking");
  ...
  function seedDuration(e, t = {}) {
    ...
    const n = Math.abs(i - f) > .05;
    f = i;
    if (n || m <= p || m > f + .2) {
      p = 0;
      m = f;
    }
    y = Math.max(p, Math.min(m, y || 0));
    show();
    buildPlaceholderFilmstrip();
    cacheTrackMetrics();
    paintChrome({
  ...
  function attach(e, t = {}) {
    ...
    c.setAttribute("controlslist", "nodownload nofullscreen noremoteplayback noplaybackrate");
    c.disablePictureInPicture = true;
    const i = Number(t.durationHint);
    f = Number.isFinite(c.duration) && c.duration > 0 ? c.duration : i > .25 ? i : 0;
    p = 0;
    m = f || 1;
    y = Number.isFinite(c.currentTime) ? c.currentTime : 0;
    ...
    flushPendingPreviewRegions();
    if (f > .25 && !(Number.isFinite(c.duration) && c.duration > 0)) {
      if (O && O.length) {
        const e = O.slice();
        O = null;
        setSplits(e);
    ...
    const kickFilmstrip = () => {
      if (!c || c !== e) return;
      f = Number.isFinite(c.duration) && c.duration > 0 ? c.duration : f;
      m = Math.max(m, f || 1);
      if (p >= m) {
        p = 0;
        m = f || 1;
    ...
    if (f > 0 && c.readyState >= 2) {
      kickFilmstrip();
    } else {
      c.addEventListener("loadedmetadata", kickFilmstrip, {
        once: true
      });
      c.addEventListener("loadeddata", kickFilmstrip, {
        once: true
      });
      setTimeout(() => {
        if (!d || c !== e) return;
        if (Number.isFinite(c.duration) && c.duration > 0) kickFilmstrip(); else if (f > .25) {
          cacheTrackMetrics();
          paintChrome({
            rebuildSegments: true
          });
  ...
  function setSplits(e) {
    ...
    for (const e of t) {
      ...
      const n = Math.max(p + .05, Math.min(m - .05, t));
      if (i.some(e => Math.abs(e - n) < .08)) continue;
      i.push(n);
    }
    i.sort((e, t) => e - t);
    const n = isRankingEdit() ? 4 : 24;
    x = i.slice(0, n);
    O = null;
    cacheTrackMetrics();
    paintChrome({
  ...
  function focusTrim(t) {
    ...
    cacheTrackMetrics();
    const i = Number.isFinite(t) ? t : y;
    const n = Math.min(2.5, Math.max(.8, f * .12));
    const r = m - p;
    if (r >= f * .95) {
      p = Math.max(0, i - n * .35);
      m = Math.min(f, Math.max(p + e, i + n * .65));
    }
    y = Math.max(p, Math.min(m, i));
    paintChrome({
      rebuildSegments: true
  ...
  function splitAt(t) {
    ...
    cacheTrackMetrics();
    let i = Number.isFinite(t) ? t : y;
    i = Math.max(p + .05, Math.min(m - .05, i));
    const n = x.findIndex(e => Math.abs(e - i) < .08);
    if (n >= 0) {
      y = x[n];
      paintChrome();
      paintSegments(y);
      scheduleSeek(y, true);
      selectSegmentAtTime(y + .04) ?? selectSegmentAtTime(y - .04);
      return true;
    }
    const r = isRankingEdit();
    const s = r ? 4 : 24;
    if (x.length >= s) {
      if (!r) {
        paintSegments(i);
      ...
      for (let e = 0; e < x.length; e++) {
        const r = Math.abs(x[e] - i);
        if (r < n) {
          n = r;
          t = e;
        }
      }
      const s = t > 0 ? x[t - 1] + e : p + e;
      const o = t < x.length - 1 ? x[t + 1] - e : m - e;
      if (o - s < e) {
        paintSegments(i);
        return false;
      }
      x[t] = Math.max(s, Math.min(o, i));
      const a = x[t];
      x.sort((e, t) => e - t);
      y = a;
      setHandlesUnlocked(true);
      ...
      paintSegments(y);
      scheduleSeek(y, true);
      selectSegmentAtTime(a + .04) ?? selectSegmentAtTime(a - .04);
      try {
        markRankingTimelineDirty();
      } catch (e) {}
      return true;
  ...
  function setSkipRegions(e) {
    ...
    for (const e of t) {
      ...
      i.push({
        start: Math.max(0, t),
        end: f > 0 ? Math.min(f, n) : n
      });
    }
    i.sort((e, t) => e.start - t.start);
    B = i;
    if (ie) ie.classList.toggle("has-silence-cuts", B.length > 0);
    paintChrome({
      rebuildSegments: true
    });
    return B.slice();
  ...
  function clearSkipRegions() {
    B = [];
    if (ie) ie.classList.remove("has-silence-cuts");
    if (d) paintChrome({
      rebuildSegments: true
    });
  }
  function setSkipRegionsPreview(e) {
    ...
    if (!d || !f || E <= 0) {
      R = i;
      if (c && Number.isFinite(c.duration) && c.duration > 0) {
        f = c.duration;
        m = Math.max(m, f);
      }
      if (d) {
    ...
    R = null;
    if (d) paintChrome(); else paintCutPreview();
    return A.slice();
  }
  ...
  function clearSkipRegionsPreview() {
    A = [];
    R = null;
    if (ne) ne.classList.remove("has-cut-preview", "has-cut-preview-add");
    const e = ne?.querySelector(".preview-timeline-cut-preview");
    if (e) e.remove();
  }
  return {
    attach: attach,
    ...
    highlightPartAt: highlightPartAt,
    getSelectedSegIndex: () => M,
...
const PreviewCtxMenu = (() => {
  ...
  function openFromEvent(e) {
    ...
    if (!(i instanceof Element)) {
      i = document.getElementById("previewTimelineWrap");
    }
    if (!(i instanceof Element)) return false;
    const n = i.closest("#previewTimelineWrap, #previewTimelineShell") || (e.clientX != null ? document.getElementById("previewTimelineWrap") : null);
    const s = i.closest("#templateVideoPreview");
    if (!n && !s) return false;
    e.preventDefault?.();
    e.stopPropagation?.();
  ...
  function onSplit() {
    ...
    try {
      const e = PreviewTimeline.getActiveEditRange?.();
      const t = PreviewTimeline.getTrim?.() || {};
      const i = Number(t.duration) || 0;
      const n = e && Number.isFinite(e.start) && Number.isFinite(e.end) ? e.end - e.start : 0;
      const r = n >= .35 && (e.manual || e.segIndex != null) && (i <= 0 || n < i * .92);
      const s = !!document.getElementById("previewTimelineShell")?.classList.contains("is-ranking-edit");
      if (r && !s && window.SolisSilencer?.applyManualCut) {
        const t = window.SolisSilencer.applyManualCut(e.start, e.end);
        if (t) {
          try {
    ...
    if (!r) {
      try {
        ...
        if (e) {
          e.hidden = false;
          e.textContent = "Move the playhead, then Cut again";
  ...
  function onImprove() {
    close();
    try {
      const e = typeof PreviewTimeline !== "undefined" && PreviewTimeline.getTrim?.() || {};
      let t = Number(e.start);
      let i = Number(e.end);
      const n = Number(e.duration) || 0;
      if (!Number.isFinite(t) || t < 0) t = 0;
      if (!Number.isFinite(i) || i <= t + .35) {
        i = n > .35 ? n : t + Math.max(n, 1);
      }
      if (window.SolisImproveClip?.openForRange) {
        window.SolisImproveClip.openForRange(t, i, "");
      } else if (window.SolisImproveClip?.open) {
        window.SolisImproveClip.open("");
      } else if (window.SolisImproveClip?.toggle) {
        window.SolisImproveClip.toggle();
      }
    } catch (e) {}
  }
  ...
  function onDocPointerDown(e) {
    ...
    if (performance.now() - i < 220) return;
    close();
  }
  function onKey(e) {
  ...
  function bindTargets() {
    ...
    document.addEventListener("contextmenu", e => {
      const t = openFromEvent(e);
      if (t) {
        try {
          const e = window.PreviewTimeline?._peekRightPending?.();
          if (e) e.openedCtx = true;
        } catch (e) {}
      }
    }, true);
...
const TimelineRangeGlass = (() => {
  ...
  function placeNear(e, t) {
    ...
    let s = (e || window.innerWidth / 2) - r.width / 2;
    let o = (t || 120) - r.height - 14;
    const a = document.getElementById("previewTimelineShell");
    if (a) {
      const e = a.getBoundingClientRect();
      o = e.top - r.height - 10;
      s = e.left + (e.width - r.width) / 2;
    }
    s = Math.max(n, Math.min(s, window.innerWidth - r.width - n));
    o = Math.max(n, o);
    i.style.left = `${Math.round(s)}px`;
  ...
  function showForRange(e, t, i) {
    ...
    try {
      if (window.SolisImproveClip?.openForRange) {
        window.SolisImproveClip.openForRange(e, t, "");
        return;
      }
      if (window.SolisImproveClip?.open) {
        window.SolisImproveClip.open("");
        return;
      }
    } catch (e) {}
...
function setConfirmUseTemplateLabel(e) {
  const t = document.getElementById("confirmUseTemplateBtn");
  if (!t) return;
  const i = String(e || "").trim() || "Use Template";
  let n = t.querySelector(".template-use-confirm-label");
  if (!n) {
    t.textContent = "";
    n = document.createElement("span");
    n.className = "template-use-confirm-label";
    t.appendChild(n);
  }
  n.textContent = i;
  const r = document.getElementById("confirmUseTemplateFab");
  if (r) {
    const e = r.querySelector(".template-use-fab-label");
    if (e) e.textContent = i;
    r.setAttribute("aria-label", i);
    r.title = i;
  }
}

window.setConfirmUseTemplateLabel = setConfirmUseTemplateLabel;

function syncLibraryConfirmLabel() {
  const e = window.clipsStudio;
  const t = document.getElementById("confirmUseTemplateBtn");
  const i = document.getElementById("confirmUseTemplateFab");
  if (!t) return;
  const n = !!e?.currentTemplateForPreview?.isLibraryPreview;
  if (n) {
    const i = !!(e._librarySplitscreenDirty || e._libraryOverlayDirty || e._libraryRankingDirty || e._librarySilenceDirty);
    setConfirmUseTemplateLabel(i ? "Apply & Download" : "Download");
    t.classList.add("library-download-mode");
    t.removeAttribute("data-solis-keyhint");
    t.removeAttribute("aria-keyshortcuts");
  } else if (!t.classList.contains("library-download-mode")) {
    if (!t.getAttribute("data-solis-keyhint")) {
      t.setAttribute("data-solis-keyhint", "Use template|Enter");
      t.setAttribute("aria-keyshortcuts", "Enter");
    }
  }
  if (typeof window.syncUseTemplateFab === "function") window.syncUseTemplateFab(); else if (i) {
    const e = i.querySelector(".template-use-fab-label");
    const n = (t.querySelector(".template-use-confirm-label")?.textContent || t.textContent || "").trim() || "Use Template";
    if (e) e.textContent = n;
    i.setAttribute("aria-label", n);
    i.title = n;
    i.classList.toggle("library-download-mode", t.classList.contains("library-download-mode"));
    if (t.classList.contains("library-download-mode")) {
      i.removeAttribute("data-solis-keyhint");
      i.removeAttribute("aria-keyshortcuts");
    } else {
      i.setAttribute("data-solis-keyhint", "Use template|Enter");
      i.setAttribute("aria-keyshortcuts", "Enter");
    }
  }
}
...
function syncUseTemplateFab() {
  ...
  if (!e || !t) return;
  const i = typeof window !== "undefined" && window.innerWidth <= 900;
  const n = e.hidden || e.style.display === "none" || e.getAttribute("data-pro-locked") === "1" || !i && window.getComputedStyle(e).display === "none";
  if (n) {
  ...
  const s = (e.querySelector(".template-use-confirm-label")?.textContent || e.textContent || "").trim() || "Use Template";
  ...
  if (e.classList.contains("library-download-mode") || !/^use template$/i.test(s)) {
    ...
    e.removeAttribute("aria-keyshortcuts");
  } else {
    t.setAttribute("data-solis-keyhint", "Use template|Enter");
    t.setAttribute("aria-keyshortcuts", "Enter");
    e.setAttribute("data-solis-keyhint", "Use template|Enter");
    e.setAttribute("aria-keyshortcuts", "Enter");
  }
}
...
function collapseSplitscreenSecondary() {
  ...
  const e = splitscreenSecondaryCollapsed && Number(splitscreenContentRatio) >= .97;
  ...
  applySplitscreenRatio();
  if (!e) markLibrarySplitscreenDirty();
}
...
const dividerInitCheck = setInterval(() => {
  ...
  if (e && e.dataset.splitscreenInit === "true" || _dividerInitTries > 20) {
...
class ClipsStudio {
  ...
  async init() {
    ...
    try {
      ...
      const i = typeof window !== "undefined" && window.innerWidth <= 768;
      if (t || !e || e === "create") {
        if (i) {
        ...
        try {
          this.switchTab(e === "create" && i ? "templates" : e);
        } catch (e) {
          this.switchTab(i ? "templates" : "create");
        }
      }
      this.moveSlider();
  ...
  bindEvents() {
    ...
    if (e) {
      ...
      e.addEventListener("input", () => {
        if (!this._isQuotaExhausted(this._limitCheckCache?.data, 1)) {
  ...
  switchTab(e) {
    if (e === "create" && typeof window !== "undefined" && window.innerWidth <= 768) {
      const e = document.querySelector("#templatesSection.active, #librarySection.active");
      if (!e) {
        document.querySelectorAll(".clips-section").forEach(e => {
          const t = e.id === "templatesSection";
        ...
        document.querySelectorAll(".clips-tab, .clips-sub-item").forEach(e => {
          ...
          e.classList.toggle("active", t === "templates");
        });
        this.currentTab = "templates";
        try {
          localStorage.setItem("clipsStudioCurrentTab", "templates");
    ...
    if (e !== "create" && typeof window.closeMobileCreateSheet === "function" && window.innerWidth <= 768) {
  ...
  openTemplatePreviewModal(e, t, i = {}) {
    ...
    if (a) {
      ...
      if (typeof this._resetPhoneMedia === "function") {
        this._resetPhoneMedia(a, `<div class="preview-skel" aria-hidden="true"></div>`);
      } else {
    ...
    if (!o && (t?.id === "splitscreen" || t?.id === "ranked_compilation")) {
      ...
      const seedCaptionsAndSuggest = () => {
        try {
          ...
          const forceTip = () => {
            try {
              if (typeof window.SolisMemory?.wasSuggestionRejected === "function" && window.SolisMemory.wasSuggestionRejected(e)) {
                return;
              }
              if (typeof window.SolisMemory?.offerFirstCaptionTip === "function") {
                const t = !!document.getElementById("subMemActions")?.classList.contains("open");
                const i = !!document.querySelector("#templateVideoPreview .sub-text-block.sub-suggest, #templateVideoPreview .sub-text-block.sub-mem-pick, .sub-mem-ghost");
                if (!t && !i && !window.__solisPendingSubMem) {
  ...
  closeTemplatePreviewModal() {
    ...
    if (o) {
      if (typeof window.setConfirmUseTemplateLabel === "function") {
        ...
        const e = o.querySelector(".template-use-confirm-label");
        if (e) e.textContent = "Use Template"; else o.textContent = "Use Template";
      }
      o.classList.remove("library-download-mode");
      o.disabled = false;
      o.style.pointerEvents = "";
  ...
  _resetPhoneMedia(e, t = null) {
    if (!e) return;
    const i = [];
    [ "improveEditBar" ].forEach(t => {
    ...
    try {
      window.SolisImproveClip?.ensureBar?.();
    } catch (e) {}
  }
  _setLibraryPreviewPlaceholder(e, t = "Loading preview...") {
    if (!e) return;
    e.classList.remove("has-video");
    this._resetPhoneMedia(e, `<div class="preview-skel" aria-hidden="true"></div>`);
    try {
  ...
  _parseDurationToSeconds(e) {
    ...
    const s = t.match(/(\d+)\s*m/i);
    const o = t.match(/(\d+(?:\.\d+)?)\s*s/i);
    if (r) n += Number(r[1]) * 3600;
    if (s) n += Number(s[1]) * 60;
    if (o) n += Number(o[1]);
    return n > 0 ? n : 0;
  }
  ...
  _ensureLibraryTimelineVisible(e = null, t = null) {
    ...
    const s = this._ensureLibraryTimelineStubVideo(i);
    let o = this._resolveLibraryDurationSeconds(r, n);
    if (!(o > .25) && this._libraryRankingTimelineState) {
      try {
    ...
    if (!(o > .25)) {
      o = this._isCurrentLibraryRanking?.() ? 40 : 15;
    }
    try {
      ...
      PreviewTimeline.seedDuration?.(o);
  ...
  openLibraryPreview(e, t, i, n = {}) {
    ...
    try {
      window.SolisMemory?.resetHabitOfferSession?.();
    } catch (e) {}
    try {
      window.SolisSilencer?.syncVisibility?.();
    } catch (e) {}
    try {
      this._ensureLibraryTimelineVisible(t, o);
    } catch (e) {}
  ...
  loadLibraryVideoPreview() {
    ...
    return this.mountLibraryPreviewVideo(e, t);
  }
  ...
  async mountLibrarySplitscreenPreview(e, t) {
    ...
    try {
      ...
      const layerReady = e => {
        ...
        if (typeof e === "object") {
          return Boolean(e.url || e.path || e.key || e.src || e.ready || e.exists || e.available);
        }
        return false;
      };
      ...
      e.classList.remove("has-video");
      if (typeof this._resetPhoneMedia === "function") {
        this._resetPhoneMedia(e, buildSplitscreenPreviewShell());
      } else {
  ...
  _mountLibraryPreviewFromUrl(e, t, i, n = {}) {
    ...
    s.style.cssText = "position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:transparent;z-index:2;display:block;visibility:visible;opacity:0;pointer-events:none;";
  ...
  async fetchSecureLibraryPreviewBlob(e, t, i = null, n = {}) {
    ...
    try {
      ...
      if (isStale()) return;
      if (!o.ok) {
        if (o.status === 404 || o.status === 409 || o.status === 425 || o.status === 202) {
          retrySoon(`http ${o.status}`);
          return;
        }
        retrySoon(`http ${o.status}`);
        return;
  ...
  _buildLocalCompanionVerdict(e, t) {
    ...
    const s = i.filter(e => !e.keep);
    const o = r.map(e => e.batch_index);
    const a = i.length;
    ...
    if (a <= 0) {
      ...
      const e = r.length;
      const t = s.length;
      const i = this._formatCompanionIndices(o) || "none";
  ...
  _clipReasonFromItem(e, t) {
    ...
    const s = scrub(n?.hook?.note || "");
    const o = scrub(n?.clip?.note || "");
  ...
  _buildLocalReasoningSteps(e, t, i) {
    ...
    const s = e.filter(e => !e.keep);
    if (i > 1) {
      if (r.length && s.length) {
        n.push(`${r.length} worth shipping, ${s.length} better hidden.`);
      } else if (r.length) {
  ...
  async _renderCompanionVerdict(e) {
    ...
    const s = this._companionBatchKey === i && !!this._companionTypedKey;
  ...
  _setCompanionReasoning(e, t) {
    ...
    const s = document.getElementById("companionChatReasonLabel");
    const o = document.getElementById("companionChatCard");
    ...
    if (e === "live") {
      ...
      if (s && !s.textContent.trim()) {
        s.textContent = "Reviewing clips";
      }
      if (o) {
        o.classList.add("is-thinking");
        o.classList.remove("is-typing");
      }
      if (a) a.hidden = false;
      return;
    }
    ...
    if (e === "done") {
      ...
      if (o) o.classList.remove("is-thinking");
      return;
    }
    i.hidden = true;
    if (n) n.hidden = true;
  ...
  _stopCompanionTypewriter() {
    ...
    this._stopCompanionReasoningSteps();
    this._setCompanionBubbleTyping(false);
    if (this._companionTaskResolve) {
      const e = this._companionTaskResolve;
      this._companionTaskResolve = null;
      try {
        e();
      } catch (e) {}
    }
  ...
  _typeCompanionMessage(e, {showActions: t = false, reasoning: i = null} = {}) {
    ...
    this._setCompanionChatCaret(false);
    this._setCompanionBubbleVisible(true);
    const o = i || this._companionVerdict?.reasoning || [ "Reviewing clips from the understanding pass." ];
    this._setCompanionReasoning("live");
    this._startCompanionReasoningSteps(o);
    this._setCompanionChatMarkState("reasoning");
    const a = Array.isArray(o) ? o.length : 1;
  ...
  _paintCompanionOptions(e) {
    ...
    const setOpt = (e, t, i, n) => {
      ...
      const s = String(t?.action || n || "keep_as_is").trim();
      e.dataset.action = s;
      const o = e.querySelector(".companion-chat-suggest-label");
      if (o) o.textContent = r; else e.textContent = r;
    };
  ...
  _paintCompanionVerdictUI(e = {}) {
    ...
    const s = !this._companionChoice && !!t.show_actions && Array.isArray(t.options) && t.options.length >= 2;
  ...
  _onCompanionReadMove(e) {
    ...
    if (!i || !n) return;
    if (n.classList.contains("is-watching-type")) return;
    const r = i.getBoundingClientRect();
    const s = Math.max(0, Math.min(1, (e.clientX - r.left) / Math.max(1, r.width)));
    const o = Math.max(0, Math.min(1, (e.clientY - r.top) / Math.max(1, r.height)));
    const a = (s - .5) * 12;
    const l = (o - .2) * 16;
    n.classList.add("is-reading");
    n.style.setProperty("--look-x", a.toFixed(2));
    n.style.setProperty("--look-y", l.toFixed(2));
    n.style.setProperty("--look-eye-scale", "1.08");
  ...
  _onCompanionReadLeave() {
    ...
    e.style.removeProperty("--look-eye-scale");
    ...
    if (!i?.classList.contains("is-typing") && !i?.classList.contains("is-thinking") && !e.classList.contains("is-watching-type")) {
      e.setAttribute("data-state", "idle");
    }
  }
  _setCompanionWatchTyping(e) {
    const t = document.getElementById("companionChatMark");
    if (t) t.classList.remove("is-watching-type");
  }
  _untypeCompanionMessage(e, t, i) {
    const n = String(e || "");
    if (!n) {
      i?.();
      return;
    ...
    let r = n.length;
    const step = () => {
      if (t !== this._companionTypeToken) return;
      if (r <= 0) {
        this._setCompanionChatText("");
        this._setCompanionChatCaret(false);
        this._setCompanionBubbleTyping(false);
      ...
      const e = r > 60 ? 8 : r > 24 ? 5 : 3;
      r = Math.max(0, r - e);
      this._setCompanionChatText(n.slice(0, r));
      this._companionTypeTimer = setTimeout(step, 7);
    };
    this._companionTypeTimer = setTimeout(step, 40);
  }
  _speakCompanionTask(e, {reasoning: t = null, thinkMs: i = null} = {}) {
    ...
    const s = document.getElementById("companionVerdictActions");
    if (s) s.hidden = true;
    const o = this._companionTextEl();
    const a = String(o?.textContent || "").trim();
    const l = document.getElementById("companionChatBubble");
    const c = !!a && l && !l.hidden;
    const runThinkThenType = () => {
      if (r !== this._companionTypeToken) return;
      const e = Array.isArray(t) && t.length ? t : [ "Looking at the timeline.", "Checking what you asked for." ];
      this._setCompanionChatText("");
      this._setCompanionChatCaret(false);
      this._setCompanionBubbleVisible(true);
      this._setCompanionReasoning("live");
      this._startCompanionReasoningSteps(e);
      this._setCompanionChatMarkState("reasoning");
      const s = i != null ? i : 850 + Math.min(1400, e.length * 420);
      const o = Date.now();
      this._companionTypeTimer = setTimeout(() => {
        if (r !== this._companionTypeToken) return;
        const e = Math.max(1, Math.round((Date.now() - o) / 1e3));
        this._companionLastReasonSecs = e;
        this._setCompanionReasoning("done", e);
        this._setCompanionChatMarkState("talking");
        this._setCompanionBubbleTyping(true);
        ...
        const step = () => {
          ...
          const e = n.length - t;
          let i = e <= 24 ? 3 : e <= 80 ? 5 : 8;
          const s = n[t];
          if (s === "\n") i = 1; else if (/[.,!?;:]/.test(s)) i = 1;
          t = Math.min(n.length, t + i);
          this._setCompanionChatText(n.slice(0, t));
          const o = n[t - 1];
          const a = o === "\n" ? 18 : /[.]/.test(o) ? 22 : /[,!?;:]/.test(o) ? 12 : 5;
          this._companionTypeTimer = setTimeout(step, a);
        };
        this._companionTypeTimer = setTimeout(step, 60);
      }, s);
    };
    return new Promise(e => {
      this._companionTaskResolve = e;
      if (c) {
        this._untypeCompanionMessage(a, r, runThinkThenType);
      } else {
        runThinkThenType();
      }
    });
  }
  _beginCompanionTask({reasoning: e = null} = {}) {
    this._stopCompanionTypewriter();
    const t = this._companionTypeToken;
    const i = document.getElementById("companionVerdictActions");
    if (i) i.hidden = true;
    const n = this._companionTextEl();
    const r = String(n?.textContent || "").trim();
    const s = document.getElementById("companionChatBubble");
    const o = !!r && s && !s.hidden;
    const startThink = () => {
      if (t !== this._companionTypeToken) return;
      const i = Array.isArray(e) && e.length ? e : [ "Scanning the clip.", "Listening for dead air." ];
      this._setCompanionChatText("");
      this._setCompanionChatCaret(false);
      this._setCompanionBubbleVisible(true);
      this._setCompanionReasoning("live");
      this._startCompanionReasoningSteps(i);
      this._setCompanionChatMarkState("reasoning");
      this._companionTaskStartedAt = Date.now();
    };
    return new Promise(e => {
      if (o) {
        this._untypeCompanionMessage(r, t, () => {
          startThink();
          e();
        });
      } else {
  ...
  _finishCompanionTask(e, {extraThinkMs: t = 280} = {}) {
    const i = String(e || "").trim();
    if (!i) return Promise.resolve();
    const n = this._companionTypeToken;
    const r = Math.max(0, t);
    return new Promise(e => {
      this._companionTypeTimer = setTimeout(() => {
        if (n !== this._companionTypeToken) {
          e();
          return;
        }
        const t = this._companionTaskStartedAt || Date.now();
        const r = Math.max(1, Math.round((Date.now() - t) / 1e3));
        this._companionLastReasonSecs = r;
        this._setCompanionReasoning("done", r);
        this._setCompanionChatMarkState("talking");
        this._setCompanionBubbleTyping(true);
        this._setCompanionChatCaret(true);
        this._setCompanionChatText("");
        ...
        const step = () => {
          ...
          if (s >= i.length) {
            this._setCompanionChatText(i);
            this._setCompanionChatCaret(false);
            this._setCompanionBubbleTyping(false);
            this._setCompanionChatMarkState("idle");
            this._syncCompanionReadingTarget(i);
            this._companionTypeTimer = null;
            e();
            return;
          }
          const t = i.length - s;
          let r = t <= 24 ? 3 : t <= 80 ? 5 : 8;
          const o = i[s];
          if (o === "\n") r = 1; else if (/[.,!?;:]/.test(o)) r = 1;
          s = Math.min(i.length, s + r);
          this._setCompanionChatText(i.slice(0, s));
          const a = i[s - 1];
          const l = a === "\n" ? 18 : /[.]/.test(a) ? 22 : /[,!?;:]/.test(a) ? 12 : 5;
          this._companionTypeTimer = setTimeout(step, l);
        };
        this._companionTypeTimer = setTimeout(step, 50);
      }, r);
    });
  }
  ...
  async _applyCompanionChoice(e) {
    ...
    if (n.has(i) || r.has(i)) {
      ...
      if (e) e.hidden = true;
      const s = this._companionAckForAction(i, t);
      const o = n.has(i);
      if (o) this._setCompanionChatMarkState("reasoning");
      try {
        if (o || r.has(i)) {
        ...
        this._setCompanionChatText(this._personalizeCompanionMessage(s));
        this._setCompanionChatMarkState("idle");
        if (i === "drop_soft") {
          this._paintCompanionHiddenRow();
    ...
    const s = i === "hide_weak" || i === "ship_best";
    this._companionChoice = s ? "hide_weak" : i;
    this._setCompanionMemoryStatus("off");
    if (s) {
    ...
    if (s || i === "review_all" || i === "keep_as_is") {
      try {
  ...
  _syncClipBatchPager(e) {
    ...
    const s = Math.max(0, n.findIndex(e => String(e.projectId || e.id) === r));
    const o = new Set((this._companionVerdict?.weak_project_ids || []).map(String));
    i.innerHTML = n.map((e, t) => {
      const i = String(e.projectId || e.id);
      const n = o.has(i) && this._companionChoice !== "hide_weak";
      return `<button type="button" class="clip-batch-dot${t === s ? " is-active" : ""}${n ? " is-weak" : ""}" data-pid="${i}" aria-label="Clip ${e.batchIndex || t + 1}"></button>`;
    }).join("");
  ...
  _stepClipBatch(e) {
    ...
    const s = t[r];
    if (s) this.openLibraryPreview(s.id, s.projectId || s.id, null, {
      fast: true
    });
  }
  ...
  _isCurrentLibrarySplitScreen() {
    ...
    const t = `${e.template || ""} ${e.templateName || ""} ${this.currentTemplateForPreview?.type || ""} ${this.currentTemplateForPreview?.templateId || ""}`.toLowerCase();
    return t.includes("splitscreen") || t.includes("split screen") || t.includes("split_screen") || t.includes("face_track") || t.includes("facetrack") || t.includes("reframe");
  }
  ...
  async _configureLibraryEditingUI() {
    ...
    if (typeof window.syncLibraryConfirmLabel === "function") {
      window.syncLibraryConfirmLabel();
    } else if (typeof window.setConfirmUseTemplateLabel === "function") {
      window.setConfirmUseTemplateLabel(this._librarySilenceDirty ? "Apply & Download" : "Download");
      t.classList.toggle("library-download-mode", true);
      if (typeof window.syncUseTemplateFab === "function") window.syncUseTemplateFab();
    } else {
      const e = t.querySelector(".template-use-confirm-label");
      const i = this._librarySilenceDirty ? "Apply & Download" : "Download";
      if (e) e.textContent = i; else t.textContent = i;
      t.classList.toggle("library-download-mode", true);
      if (typeof window.syncUseTemplateFab === "function") window.syncUseTemplateFab();
    }
  ...
  async confirmTemplateUse() {
    ...
    if (i && n) {
      ...
      if (e) {
        ...
        e.disabled = true;
        const t = (e.querySelector(".template-use-confirm-label")?.textContent || e.textContent || "").trim();
        e.dataset.prevLabel = t || "Download";
        const i = s || a || l || c ? "Applying…" : "Downloading…";
        if (typeof window.setConfirmUseTemplateLabel === "function") {
      ...
      try {
        ...
        if (e) {
          ...
          const t = e.dataset.prevLabel || "Download";
          if (typeof window.setConfirmUseTemplateLabel === "function") {
    ...
    if (Array.isArray(this._multiTaskBatchUrls) && this._multiTaskBatchUrls.length) {
      ...
      if (o && this.isValidMediaUrl(o)) {
        this.addMultiTaskUrl(o);
        a = this.getMultiTaskUrls();
      }
    } else if (o) {
      a = [ o ];
    ...
    if (a.length > 1) {
      try {
        ...
        const s = Number(e?.concurrent_cap || this._multiTaskBatchMax() || 1);
        if (r + t > s) {
          showNotification(`Your plan allows ${s} at once — you have ${r} live and selected ${t}`, "warning");
          return;
        }
      } catch (e) {}
      this.closeTemplatePreviewModal();
      ...
      try {
        ...
        if (!Number.isNaN(r.getTime())) {
          if (r.getUTCHours() === 0 && r.getUTCMinutes() === 0 && r.getMinutes() === 0 && r.getHours() > 0 && r.getHours() <= 6) {
            ...
            const s = Math.round((r - n) / 864e5);
            if (s === 0) return `today at ${i}`;
            if (s === 1) return `tomorrow at ${i}`;
          ...
          const s = Math.round((n - i) / 864e5);
          if (s === 0) return `today at ${t}`;
          if (s === 1) return `tomorrow at ${t}`;
  ...
  async startClipProcessingWithSlots(e, t, i = {}) {
    ...
    try {
      ...
      s = Number.isFinite(a) ? Date.now() + a + 1 : Date.now();
      ...
      if (l) {
        ...
        const s = r.title || (e ? "YouTube video" : "Your video");
        const o = {
        ...
        if (!n || i.batchIndex === 0 || !l.optimisticPending) {
          l.beginOptimisticGeneration("Starting...", t, o);
        }
      }
      const c = {
        id: s,
      ...
      if (g.status === 401) {
        this._rollbackOptimisticStart(s, "Session expired. Please try again.");
      ...
      if (!g.ok) {
        ...
        if (g.status === 429) {
          if (e.error_code === "CONCURRENT_GENERATION_BLOCKED") {
            this._rollbackOptimisticStart(s, e.error || "Another generation is already running.");
            showNotification(e.error, "error");
            return;
          }
          if (e.error_code === "GENERATION_COOLDOWN") {
            this._rollbackOptimisticStart(s, e.error || "Please wait before starting another generation.");
          ...
          if (e.error_code === "MAX_EFFORT_LIMIT_REACHED") {
            this._rollbackOptimisticStart(s, e.error || "Max effort daily limit reached. Switch to Normal effort.");
          ...
          if (e.error_code === "DAILY_LIMIT_REACHED" || e.error_code === "MONTHLY_LIMIT_REACHED") {
            this._rollbackOptimisticStart(s, e.error || "Generation limit reached — upgrade or wait for reset.");
          ...
          if (e.error_code === "VIDEO_LIMIT_REACHED") {
            this._rollbackOptimisticStart(s, e.error || "Library is full — free a slot or upgrade.");
          ...
          this._rollbackOptimisticStart(s, e.error || "Rate limit reached. Please try again later.");
        ...
        if (g.status === 503 && e.error_code === "SERVER_AT_CAPACITY") {
          ...
          this._rollbackOptimisticStart(s, t);
          ...
          this._rollbackOptimisticStart(s, n);
      ...
      const b = this.processingItems.find(e => e.id === s);
      if (b) {
      ...
      this._rollbackOptimisticStart(s, e?.message ? `Failed to start: ${e.message}` : null);
  ...
  _isQuotaExhausted(e, t = 1) {
    if (!e || typeof e !== "object") return false;
    const i = Math.max(1, Number(t) || 1);
    const n = Number(e.daily?.remaining);
    const r = Number(e.monthly?.remaining);
    const s = String(e.block_reason || "").toLowerCase();
    if (s === "daily_limit" || s === "monthly_limit") return true;
    if (e.daily_limit_reached === true) return true;
    if (Number.isFinite(n) && n < i) return true;
    if (e.monthly_limit_reached === true) return true;
    if (e.monthly?.limit > 0 && Number.isFinite(r) && r < i) return true;
    if (e.can_generate === false && (s === "daily_limit" || s === "monthly_limit" || s === "quota")) {
      return true;
    }
    return false;
  }
  _blockQuotaExhausted(e = null) {
    try {
      if (e && typeof window.updateUrlQuotaRail === "function") {
        window.updateUrlQuotaRail(e);
      }
    } catch (e) {}
    try {
      sessionStorage.removeItem("solis_quota_rail_dismiss");
  ...
  _scheduleAutoSubmitFromPaste(e = {}) {
    ...
    this._pasteAutoSubmitT = setTimeout(() => {
      try {
        ...
        const t = document.getElementById("processUrlBtn");
        if (t?.classList.contains("is-generating") || t?.classList.contains("is-upgrade-cta")) {
          if (t.classList.contains("is-upgrade-cta")) {
            this.openUrlSubmitUpgrade();
          }
          return;
        ...
        if (this._isQuotaExhausted(this._limitCheckCache?.data, 1)) {
          this._blockQuotaExhausted(this._limitCheckCache.data);
          return;
        }
  ...
  async processYouTubeUrl() {
    ...
    const e = document.getElementById("processUrlBtn");
    if (e?.classList.contains("is-upgrade-cta")) {
      this.openUrlSubmitUpgrade();
      return;
    }
    const t = e?.classList.contains("is-generating");
    if (t && !this._multiTaskActive()) {
    ...
    const i = Date.now();
    if (i - this.lastYouTubeProcessTime < CONFIG.RATE_LIMITING.YOUTUBE_PROCESS_MIN_MS) {
      showNotification("Please wait a moment before trying again", "warning");
      return;
    }
    this.lastYouTubeProcessTime = i;
    ...
    try {
      ...
      if (!s) {
        try {
          s = await Promise.race([ r, new Promise(e => setTimeout(() => e(null), 2500)) ]);
        } catch (e) {
          s = null;
        }
      } else {
      ...
      if (!s) {
        try {
          s = await this._getCachedLimitCheck(true);
        } catch (e) {
          s = null;
        }
      }
      const o = e.length;
      if (!s) {
        showNotification("Couldn't verify your upload limit. Stay here and try again in a moment.", "warning");
        return;
      }
      try {
        if (typeof window.updateUrlQuotaRail === "function") {
          window.updateUrlQuotaRail(s);
        }
      } catch (e) {}
      if (this._isQuotaExhausted(s, o)) {
        if (o > 1) {
          const e = Number(s.daily?.remaining);
          const t = Number(s.monthly?.remaining);
          const i = Number.isFinite(e) ? Math.max(0, e) : Number.isFinite(t) ? Math.max(0, t) : 0;
          showNotification(`Multi Task needs ${o} uploads — you have ${i} left`, "warning");
        }
        this._blockQuotaExhausted(s);
        return;
      }
      {
        const e = s.max_effort;
        if (e && e.limit > 0 && e.remaining <= 0 && typeof window.getSelectedEffortMode === "function" && window.getSelectedEffortMode() === "max") {
          if (typeof window.setSelectedEffortMode === "function") {
            window.setSelectedEffortMode("normal");
          }
        }
        if (!s.can_generate) {
          if (s.block_reason === "library_full" || s.block_reason === "storage_full") {
            this._notifyGenerationBlock(s);
            if (typeof window.syncStorageLimitsFromStatus === "function") {
              window.syncStorageLimitsFromStatus(s);
            }
          } else if (s.is_generating || s.block_reason === "in_progress") {
            const e = Number(s.concurrent_live || 0);
            const t = Number(s.concurrent_cap || 1);
            if (e >= t) {
          ...
          } else if ((s.generation?.cooldown_remaining_seconds || 0) > 0) {
            this._notifyGenerationBlock(s);
          }
        }
      }
      ...
      const c = typeof window.syncStorageLimitsFromStatus === "function" ? window.syncStorageLimitsFromStatus(s) : null;
