        ...
        async function initPaddle() {
            ...
            // Initialize PaddleManager (fetches config from backend)
        ...
        window.handleCheckoutSuccess = async function(checkoutData) {
            ...
            setTimeout(() => {
                window.location.href = '/premium?payment=success&plan=' + encodeURIComponent(result.plan || planName);
            }, 1800);
        };
        ...
        async function checkAuthenticationAndInit() {
            try {
                ...
                // Check with backend using httpOnly cookie authentication
                const authUrl = typeof window.apiUrl === 'function'
                    ? window.apiUrl('/api/auth/check')
                    : '/api/auth/check';
                ...
                if (response.ok) {
                    ...
                    if (data.authenticated && data.user) {
                        // User is authenticated, store for later use
          ...
          try {
            ...
            if (user && user.email) {
                ...
                if (typeof window.applyPremiumCurrentPlan === 'function') {
                    window.applyPremiumCurrentPlan(planRaw);
                }
            } else {
