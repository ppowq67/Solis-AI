class FloatingCustomizeBar {
    ...
    init() {
        ...
        // ─── Click-outside: only close when clicking truly outside both the pill AND the preview ───
        // We use bubbling (not capture) so that internal stopPropagation calls still work.
        document.addEventListener('click', (e) => {
            ...
            // If click is inside the pill, always keep open
            if (pill.contains(e.target)) return;

            // If click is inside the preview area, keep open — user is selecting a new element
            const preview = document.getElementById('templateVideoPreview');
            if (preview && preview.contains(e.target)) return;

            // Anything else (outside both pill and preview) → close
            this.isClosing = true;
            this.hidePanel();
    ...
    createPanel() {
        ...
        // ─── Fixed to bottom-right corner, always ───
        pill.style.cssText = `
            position: fixed;
            bottom: 24px;
            right: 24px;
        ...
        const closePill = () => {
            ...
            // ─── Reset to bottom-right anchor (never clear these) ───
    ...
    showPanel(element) {
        // Ranking preview is owned by RankingTextPill — never open legacy #pill
        const preview = document.getElementById('templateVideoPreview');
        ...
        // ─── Always bottom-right; never touches top/left ───
    ...
    attachClickListeners() {
        ...
        preview.addEventListener('mouseup', (e) => {
            ...
            // Ranking uses RankingTextPill (subtitle-style) — never open legacy #pill
            ...
            if (target && target !== preview && !target.closest('.solis-watermark')) {
                if (target.textContent?.trim() && target.textContent.trim().length < 200) {
                    ...
                    if (isShiftHeld && this.currentElement) {
                        ...
                        // ─── showPanel no longer needs x/y coords ───
...
const customizer = (() => {
    ...
    window.initializeFloatingCustomizer = (reinitialize = false) => {
        ...
        const pill = document.getElementById('pill');
        if (pill) {
            // Ensure bottom-right positioning is always set on init
            pill.style.position = 'fixed';
            pill.style.bottom   = '24px';
            pill.style.right    = '24px';
            pill.style.top      = 'auto';
