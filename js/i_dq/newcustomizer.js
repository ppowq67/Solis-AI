class CustomizerPro {
  constructor() {
    ...
    this.scrubbers = {};
    this.ALLOWED_FONTS = [ "'Luckiest Guy', cursive", "'Playfair Display', serif", "'Fraunces', serif", "'Syne', sans-serif", "'Plus Jakarta Sans', sans-serif", "'Space Grotesk', sans-serif", "'Bricolage Grotesque', sans-serif", "'Outfit', sans-serif", "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", "'DM Serif Display', serif", "'Abril Fatface', serif", "'Bebas Neue', sans-serif", "'Urbanist', sans-serif", "'Poppins', sans-serif", "'Manrope', sans-serif", "'Raleway', sans-serif", "'Montserrat', sans-serif", "'Quicksand', sans-serif", "'Courier New', monospace", "'JetBrains Mono', monospace" ];
    this.ALLOWED_COLORS = [ "#1d1d1f", "#007AFF", "#FF2D55", "#AF52DE", "#FF9500", "#34C759", "#5856D6", "#FF3B30", "#000000", "#FFFFFF" ];
    this.init();
  }
  normalizeColor(t) {
    if (!t) return "transparent";
    t = t.trim();
    if (t.startsWith("#") || t === "transparent") {
      return t.toLowerCase();
    }
    const e = t.match(/rgba?\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)/);
    if (e) {
      const t = parseInt(e[1]).toString(16).padStart(2, "0");
      const s = parseInt(e[2]).toString(16).padStart(2, "0");
      const i = parseInt(e[3]).toString(16).padStart(2, "0");
      return `#${t}${s}${i}`.toLowerCase();
    }
    return t.toLowerCase();
  }
  validateColor(t, e = "#1d1d1f") {
    ...
    console.warn(`[XSS Prevention] Invalid color rejected: ${t}`);
    return e;
  }
  validateNumber(t, e = 0, s = 999, i = 0) {
    const r = parseInt(t, 10);
    if (isNaN(r)) {
      console.warn(`[XSS Prevention] Invalid number rejected: ${t}`);
      return i;
    }
    return Math.max(e, Math.min(s, r));
  }
  validateFont(t, e = "'Plus Jakarta Sans', sans-serif") {
    if (typeof t !== "string") return e;
    if (this.ALLOWED_FONTS.includes(t)) {
      return t;
    }
    console.warn(`[XSS Prevention] Font not in whitelist: ${t}`);
    return e;
  }
  sanitizeText(t) {
  ...
  attachDoubleClickListeners() {
    if (this.panel) {
      ...
      this.navBtns.forEach(t => t.classList.remove("active"));
    }
  }
  closeCustomizer() {
    try {
      if (this.panel) {
        ...
        this.closeTimeout = setTimeout(() => {
          try {
            ...
            console.warn("[CustomizerPro] Error closing:", t);
            this.isAnimating = false;
          }
        }, 350);
      }
    } catch (t) {
      console.warn("[CustomizerPro] Close error:", t);
    }
  }
  selectElement(t) {
    ...
    const s = e.fontFamily || this.state.font;
    this.elementOriginalFont = s;
    const i = e.textShadow;
    if (i && i !== "none") {
      this.elementOriginalShadow = i;
    } else {
      this.elementOriginalShadow = null;
    }
    if (this.ALLOWED_FONTS.some(t => s.includes(t.replace(/['"]/g, "").split(",")[0]))) {
      const t = s.split(",")[0].trim().replace(/['"]/g, "");
      const e = this.ALLOWED_FONTS.find(e => e.includes(t));
      if (e) {
        this.state.font = e;
      }
    }
  ...
  syncStateFromElement() {
    ...
    try {
      const t = window.getComputedStyle(this.currentElement);
      this.state.textColor = this.normalizeColor(t.color || this.state.textColor);
      this.state.weight = this.validateNumber(parseInt(t.fontWeight) || 400, 100, 900, 400);
      this.state.size = this.validateNumber(parseInt(t.fontSize) || 16, 8, 200, 40);
      this.state.spacing = this.validateNumber(parseInt(t.letterSpacing) || 0, -50, 50, 0);
      ...
      console.warn("[CustomizerPro] Error syncing state:", t);
    }
  }
  updateFontUI() {
  ...
  updateColorUI() {
    ...
    const e = this.normalizeColor(this.state.textColor);
    t.forEach(t => {
      const s = window.getComputedStyle(t).backgroundColor;
      const i = this.normalizeColor(s);
      if (i === e) {
        t.classList.add("customizer-active");
      }
    });
  ...
  updateScrubberUI() {
    if (this.scrubbers.weight) {
      const t = [ 100, 200, 300, 400, 500, 600, 700, 800, 900 ];
      const e = t.indexOf(this.state.weight);
      if (e >= 0) this.scrubbers.weight.setIndex(e);
    }
  }
  ensureDOM() {
  ...
  positionNearElement() {
    ...
    const t = this.currentElement.getBoundingClientRect();
    const e = 360;
    const s = 380;
    const i = 60;
    const r = 12;
    ...
    if (t.classList.contains("active")) {
      ...
      this.closeTimeout = setTimeout(() => {
        try {
          ...
          console.warn("[CustomizerPro] Error closing panel:", t);
          this.isAnimating = false;
        }
      }, 350);
    } else {
      ...
      try {
        ...
        this.positionNearElement();
        this.navBtns.forEach(t => t.classList.remove("active"));
        this.views.forEach(t => t.classList.remove("active"));
        t.classList.add("active");
        this.titleEl.innerText = s;
        const i = document.getElementById(`customizer-view-${e}`);
        ...
        this.expandTimeout = setTimeout(() => {
          try {
            ...
            console.warn("[CustomizerPro] Error completing expand:", t);
            this.isAnimating = false;
          }
        }, 450);
      } catch (t) {
        console.warn("[CustomizerPro] Error opening panel:", t);
        this.isAnimating = false;
      }
    }
  }
  ...
  setupScrubbers() {
    const t = [ 100, 200, 300, 400, 500, 600, 700, 800, 900 ];
    const e = [ 12, 16, 20, 24, 28, 32, 36, 40 ];
    const s = [ -20, -15, -10, -5, 0, 5, 10, 15, 20 ];
    const i = [ 0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50 ];
    const r = [ 0, 5, 10, 15, 20, 25, 30, 35, 40 ];
    const n = [ 0, 5, 10, 15, 20, 25 ];
  ...
  createScrubber({container: t, label: e, presets: s, defaultValue: i, onChange: r}) {
    ...
    p.className = "customizer-scrubber-label";
    p.style.userSelect = "none";
    p.style.WebkitUserSelect = "none";
    const h = document.createElement("span");
    h.className = "customizer-lbl";
    h.textContent = e;
    p.appendChild(h);
    const y = document.createElement("span");
    y.className = "customizer-val";
    y.id = `scrubber-val-${e.replace(/\s/g, "-")}`;
    p.appendChild(y);
    const f = document.createElement("div");
    f.className = "customizer-scrubber-track";
    const g = document.createElement("div");
    g.className = "customizer-scrubber-glow";
    f.appendChild(g);
    const b = document.createElement("div");
    b.className = "customizer-scrubber-dots";
    f.appendChild(b);
    const v = document.createElement("div");
    v.className = "customizer-scrubber-handle";
    const x = document.createElement("div");
    x.className = "customizer-s-grip";
    const w = document.createElement("div");
    w.className = "customizer-s-grip-bar";
    w.style.height = "10px";
    x.appendChild(w);
    const E = document.createElement("div");
    E.className = "customizer-s-grip-bar";
    E.style.height = "7px";
    x.appendChild(E);
    v.appendChild(x);
    f.appendChild(v);
    ...
    const updateUI = () => {
      ...
      if (c && t > 2 && t < 98) {
        B.style.borderRadius = u === "right" ? "10px 9999px 9999px 10px" : "9999px 10px 10px 9999px";
      } else {
        B.style.borderRadius = "9999px";
      }
      k.style.flexDirection = u === "left" ? "row-reverse" : "row";
      F[0].style.height = c ? "20px" : "10px";
      F[1].style.height = c ? "13px" : "7px";
  ...
  setupFonts() {
    ...
    const e = document.getElementById("customizer-font-list");
    Object.entries(t).forEach(([t, s]) => {
      const i = document.createElement("div");
      i.style.cssText = "font-size:11px;font-weight:700;color:#999;text-transform:uppercase;letter-spacing:0.5px;margin-top:12px;margin-bottom:6px;padding:0 8px;";
      i.textContent = t;
      e.appendChild(i);
      s.forEach(t => {
        const s = document.createElement("div");
        s.className = `customizer-font-item ${this.state.font === t.family ? "customizer-active" : ""}`;
        s.style.userSelect = "none";
        s.style.WebkitUserSelect = "none";
        const i = document.createElement("span");
        i.textContent = t.name;
        i.style.fontFamily = t.family;
        i.style.fontWeight = "600";
        i.style.fontSize = "16px";
        i.style.letterSpacing = "0.2px";
        s.appendChild(i);
        s.onclick = e => {
          e.preventDefault();
  ...
  setupColors() {
    const t = [ "#1d1d1f", "#FFFFFF", "#007AFF", "#FF9500" ];
    const e = [ "#FFFFFF", "#000000", "#007AFF", "#FF9500", "#34C759", "#FF3B30" ];
    this.setupColorPalette("customizer-text-colors", "textColor", t, false);
    this.setupColorPalette("customizer-text-bg-colors", "textBg", e, true);
  }
  getColorName(t) {
    const e = {
      "#1d1d1f": "Dark Gray",
    ...
    return e[t.toUpperCase()] || "Color";
  }
  setupColorPalette(t, e, s, i) {
    const r = document.getElementById(t);
    s.forEach(t => {
      const s = document.createElement("div");
      s.className = "customizer-color-dot";
      s.style.background = t;
      s.style.userSelect = "none";
      s.style.WebkitUserSelect = "none";
      s.title = this.getColorName(t);
      s.addEventListener("click", i => {
        i.preventDefault();
        i.stopPropagation();
        const n = this.validateColor(t);
        this.state[e] = n;
        if (e === "textBg") {
    ...
    n.addEventListener("click", t => {
      ...
      const handleChange = t => {
        const i = this.validateColor(t.target.value, "#ffffff");
        this.state[e] = i;
        if (e === "textBg") {
          this.state.textBgBlur = false;
          this.hideBlurScrubber();
      ...
      s.addEventListener("change", handleChange.bind(this), false);
      s.addEventListener("cancel", handleCancel, false);
      s.click();
    }, false);
    r.appendChild(n);
    if (i) {
      const t = document.createElement("div");
      t.className = "customizer-color-dot";
      t.style.cssText = "background:white;border:2px dashed #bbb;position:relative;cursor:pointer;user-select:none;-webkit-user-select:none;";
      const e = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      e.setAttribute("width", "16");
      e.setAttribute("height", "16");
      e.setAttribute("viewBox", "0 0 18 18");
      ...
      i.setAttribute("x1", "5");
      i.setAttribute("y1", "13");
      i.setAttribute("x2", "13");
      i.setAttribute("y2", "5");
      e.appendChild(i);
      t.appendChild(e);
  ...
  getSmartTextColor(t) {
    ...
    if (e.length !== 6) return "#1d1d1f";
    const s = parseInt(e.substr(0, 2), 16), i = parseInt(e.substr(2, 2), 16), r = parseInt(e.substr(4, 2), 16);
    return (.299 * s + .587 * i + .114 * r) / 255 > .5 ? "#1d1d1f" : "#fff";
  }
  syncTextColorDot() {
    ...
    t.querySelectorAll(".c8g").forEach(t => {
      ...
      if (e.startsWith("rgb")) {
        const t = e.match(/\d+/g);
        if (t && t.length >= 3) e = "#" + t.slice(0, 3).map(t => (+t).toString(16).padStart(2, "0")).join("");
      }
      t.classList.toggle("customizer-active", e === this.state.textColor.toLowerCase());
    });
  }
  ensureBlurScrubber() {
    const t = document.getElementById("customizer-scrubber-blur-wrap");
    t.style.display = "";
    if (!this.scrubbers.blur) {
      const e = [ 2, 4, 6, 8, 10, 12, 16, 20, 24 ];
      this.scrubbers.blur = this.createScrubber({
        ...
        defaultValue: 6,
        onChange: t => {
          this.state.textBgBlurStrength = t;
          this.applyStyles();
        }
      });
    }
    ...
    const closeHandler = t => {
      ...
      if (e.classList.contains("active")) {
        try {
          ...
          console.warn("[CustomizerPro] Error in close handler:", t);
        }
      }
    };
  ...
  setupPresets() {
    const t = [ {
      ...
      state: {
        ...
        radius: 16,
        padding: 18,
        textBg: "rgba(255,255,255,0.55)",
        textBgBlur: true,
        textColor: "#1d1d1f"
      }
    } ];
    const e = document.getElementById("customizer-presets-grid");
    t.forEach((t, s) => {
      const i = document.createElement("div");
      i.className = "customizer-preset-card";
      ...
      r.className = "customizer-preset-preview";
      r.style.fontFamily = t.state.font;
      r.style.fontWeight = t.state.weight;
      r.style.fontSize = "14px";
      r.style.letterSpacing = `${t.state.spacing}px`;
      r.style.color = t.state.textColor;
      r.style.backgroundColor = t.state.textBg === "transparent" ? "transparent" : t.state.textBg;
      r.style.borderRadius = `${t.state.radius}px`;
      r.style.padding = `${t.state.padding / 3}px ${t.state.padding / 2}px`;
      r.style.minHeight = "40px";
      r.style.display = "flex";
      r.style.alignItems = "center";
      r.style.justifyContent = "center";
      r.style.width = "100%";
      r.style.maxWidth = "120px";
      r.textContent = "Sample";
      const n = document.createElement("div");
      n.className = "customizer-preset-label";
  ...
  applyStyles() {
    const t = this.elementOriginalFont || this.state.font;
    const e = this.validateFont(this.state.font, t);
    const s = this.validateNumber(this.state.weight, 100, 900, 400);
    ...
    let d = this.elementOriginalShadow || this.state.textShadow || "none";
    if (d !== "none" && u > 0) {
      const t = d.match(/^([^,]+(?:,[^,]+)*)$/);
      if (t) {
        d = `${d}, ${u}px ${u}px ${u * 1.5}px rgba(0,0,0,${.2 + u * .04})`;
      }
    }
    if (this.rankingSelectedElements && this.rankingSelectedElements.size > 0) {
      this.rankingSelectedElements.forEach(t => {
        t.style.setProperty("font-family", e, "important");
        t.style.setProperty("font-weight", s.toString(), "important");
        t.style.setProperty("font-size", `${i}px`, "important");
        ...
        if (this.state.textBgBlur || a !== "transparent") {
          ...
          if (this.state.textBgBlur) {
            ...
            t.style.setProperty("backdrop-filter", `blur(${c}px)`, "important");
            t.style.setProperty("-webkit-backdrop-filter", `blur(${c}px)`, "important");
    ...
    this.currentElement.style.setProperty("font-family", e, "important");
    this.currentElement.style.setProperty("font-weight", s.toString(), "important");
    this.currentElement.style.setProperty("font-size", `${i}px`, "important");
    this.currentElement.style.setProperty("letter-spacing", `${r}px`, "important");
    this.currentElement.style.setProperty("color", n, "important");
    this.currentElement.style.setProperty("text-shadow", d, "important");
    if (this.state.textBgBlur) {
      ...
      this.currentElement.style.setProperty("backdrop-filter", `blur(${c}px)`, "important");
  ...
  async saveCustomizations(t, e) {
    ...
    try {
      ...
      const i = await fetch(`/api/ranking-config/customize/${t}/${e}`, {
        method: "POST",
        headers: {
      ...
      if (!i.ok) {
        console.error("[CustomizerPro] Failed to save customizations:", i.statusText);
        return false;
      }
      return true;
    } catch (t) {
      console.error("[CustomizerPro] Error saving customizations:", t);
      return false;
    }
  }
}
