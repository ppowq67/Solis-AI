...
// ===== GLOBAL API REQUEST DEDUPLICATION CACHE =====
// Prevents identical requests from being sent multiple times within a short window
// This fixes the issue of multiple JS files calling the same endpoint simultaneously
window.apiRequestCache = {
    cache: new Map(), // { url_method: { promise, timestamp, data, response } }
    DEFAULT_TTL_MS: 5000,

    ttlFor(url, method = 'GET') {
        const u = String(url || '');
        if (method !== 'GET') return 0;
        if (u.includes('/clips/projects')) return 15000;
        if (u.includes('/clips/status')) return 4000;
        if (u.includes('/portal/activity')) return 30000;
        if (u.includes('/user/profile')) return 20000;
        if (u.includes('/auth/subscription')) return 20000;
        if (u.includes('/clips/preview') || u.includes('/clips/poster')) return 60000;
    ...
    async dedupFetch(url, options = {}) {
        ...
        const now = Date.now();
        const ttl = this.ttlFor(url, method);
        
        // Check if we have a cached request that's still fresh
        const cached = this.cache.get(cacheKey);
        if (cached && ttl > 0 && (now - cached.timestamp) < ttl) {
            // Always return a new Response clone from cached data to avoid "body stream already read" error
            if (cached.data) {
            ...
            // If data not yet cached, wait for the original promise and clone the response
            if (cached.response) {
                return cached.response.clone();
            }
        }
        
        // New request - create promise and cache it
        const fetchPromise = fetch(url, options);
        this.cache.set(cacheKey, { promise: fetchPromise, timestamp: now, response: null, data: null });
        
        try {
            const response = await fetchPromise;
            // Store response for future cloning
            const cached = this.cache.get(cacheKey);
            if (cached) cached.response = response.clone();
            
            // Cache parsed data for future use (for GET requests)
            if (response.ok && !options.method && method === 'GET') {
                try {
            ...
            for (const [key, data] of this.cache.entries()) {
                if ((now - data.timestamp) > this.ttlFor(key.split(':').slice(1).join(':')) * 2) {
...
function handleUpgradeCardVisibility() {
    ...
    async function fetchAndManageUpgradeCards() {
        try {
            // Use global helper from menu.js to avoid redundant fetches
            const data = await getOrFetchCurrentUser();
